...
現在は **sqlite / postgres / mysql / HTTP API（Redash・Kibana 等）** の 4 connector に対応する（Phase A2）。承認フロー・監査・delivery は connector 非依存で共通化してあり、新しいデータソースは Connector protocol への adapter 追加で増やしていく。ブラウザ操作系の管理ツール（API なし）は「クエリ→行データ」モデルに乗らないため別設計として切り出している。
...
SQL を持たない http tool では、SQL の代わりに request spec（method / path / records_path 等を宣言した JSON）を渡す。

安全設計の要点:

- **connector 別の read-only 防御**: sqlite は三重防御（read-only 接続 + `PRAGMA query_only` + authorizer）。postgres / mysql は「DB 側の read-only 専用 role（第一防御）+ sqlglot による静的 SQL 検査（単一 SELECT・relation allowlist・未知関数拒否）+ session read-only」の組み合わせ。http は endpoint allowlist + URL canonicalize + リダイレクト拒否 + レスポンスサイズ上限
- **single-use 承認**: 1 回の承認で実行できるのは 1 回だけ。承認後に SQL / request spec や配送先を変更すると digest 不一致で拒否される
- **結果データ非保持**: 結果は delivery 先に引き渡して破棄する。監査ログには値を含まないメタデータだけが残る。credential は driver 例外に混ざっても sanitize され、stdout/stderr/監査には出ない
- **doctor で事前診断**: リモート DB / API を登録したら `doctor` で接続疎通・read-only role・delivery を実データに触れず診断できる（read-only role が SELECT 専用かを introspection で機械検証する）
...
| tool-query Phase A2 | マルチコネクタ（postgres / mysql / HTTP API）+ 静的 SQL 検査 + doctor 前倒し | 完了 |
| tool-query Phase B | 監査 reconcile、Slack delivery、Correction Mining、ブラウザ操作系ツール（別設計） | 予定 |
