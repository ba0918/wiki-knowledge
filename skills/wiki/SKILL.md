...
- ingest 未実行の場合は `wiki ingest` を先に案内する
...
| `{slug}-business-rules` | rules 候補 + tests 補強 | rules, tests |
| `{slug}-state-machines` | state 候補あり | state |
| `{slug}-glossary` | 用語5語以上 | 全カテゴリ |

フロントマター: page-template.json 準拠。`type: "wiki"`、tags に `discover` を含める。`source_refs` には `raw/files/{slug}/repo-inventory.md` を含める。コード由来の事実は本文内に `path@8hash` 形式の出典を付ける。
...
1. `wikilink_render.py --write {wiki_root}/concepts/` で wikilink を GitHub 互換形式に変換
2. log.md に追記: `python3 scripts/log_append.py discover --wiki-root {wiki_root} --slug {slug} --articles N`
...
- 生成した記事には保存前に `python3 scripts/security_scan.py` を適用（secret 漏れ検出）
