...
class FlowSelectorError(Exception):
    """capability API が要求 locator を見つけられなかった（selector drift の検出線）。
...
def sanitize_exception(exc: BaseException) -> BrowserReason:
    ...
    if isinstance(exc, FlowSelectorError):
        return BrowserReason.SELECTOR_NOT_FOUND
...
class FlowContext:  # pragma: no cover - smoke（capability API は実 page を包む）
    ...
    def goto(self, route_id: str, **query) -> None:
        origin = self._entry.account.origin.rstrip("/")
        # origin は常に catalog（named route）。パラメータは query 値として渡す
        # （param→origin 直結は構造的に不可能。値バインディング）。
        path = "/" + route_id.strip("/")
        if query:
            from urllib.parse import urlencode

            path = path + "?" + urlencode({k: str(v) for k, v in query.items()})
        self._page.goto(origin + path, wait_until="networkidle")

    def read_filter(self, label: str, param: str) -> None:
        """フィルタ表示を読み戻して filter_readback の証拠にする。"""

        self._readbacks[param] = self._page.get_by_label(label).inner_text().strip()

    def read_ui_total(self, role: str, name: str) -> None:
        """UI 上の total 件数（独立 oracle）を読む。数値でなければ None のまま。"""

        text = self._page.get_by_role(role, name=name).inner_text().strip()
        try:
            self._ui_total = int(text)
        except ValueError:
            self._ui_total = None

    def read_tenant(self) -> None:
        """画面の tenant マーカー（identity oracle）を読む。抽出データ由来の値。"""

        self._account_id = self._page.locator("[data-tenant]").first.get_attribute(
            "data-tenant"
        )

    def download_csv(self, role: str, name: str) -> None:
        """role + accessible name の export ボタンから CSV を download し行を取り込む。

        ボタンが存在しなければ :class:`FlowSelectorError`（selector_not_found）。
        サーバー指定 filename は使わず bytes を封じ、CSV を rows/columns に取り込む。
        """

        button = self._page.get_by_role(role, name=name)
        if button.count() == 0:
            raise FlowSelectorError("export ボタンが見つからない")
        with self._page.expect_download() as dl:
            button.click()
        download = dl.value
        data = Path(download.path()).read_bytes()
        self._artifact_bytes = data
        import csv as _csv
        import io as _io

        rows = list(_csv.reader(_io.StringIO(data.decode("utf-8"))))
        if rows:
            self._columns = tuple(rows[0])
            self._rows = tuple(tuple(r) for r in rows[1:])
    ...
    def build_result(self, extracted_at: str) -> ExtractionResult:
        return ExtractionResult(
            ...
            # account_id は画面から読んだ tenant（read_tenant）。catalog 値では
            # 別 tenant 同型画面を検出できないため、抽出データ由来の値を使う。
