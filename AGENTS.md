...
- catalog 登録済みデータソース（PoC: sqlite）への探索的アドホック集計を dry-run 承認フローで実行する（スキル: wiki-tool-query）
- **実行契約の真実源は git 管理 catalog**: `.wiki/tools/catalog.json`（schema: `.wiki/schema/tool-catalog-schema.json`）。接続先・relation allowlist・出力上限・delivery 先を宣言し、変更は PR レビューを経る（Wiki 記事の編集では安全境界を変更できない）
...
- 実行: `python3 skills/wiki/scripts/tool_query_run.py {catalog-validate|prepare|approve|execute} --wiki-root .wiki ...`（exit: 0=成功 / 1=拒否・失敗 / 2=usage / 130=SIGINT。Python 3.11+ 必須）
- enforcement: read-only 三重防御（ro URI + query_only + authorizer）+ setlimit + 出力上限 + 全 segment symlink 拒否のパス封じ込め + CSV 無害化（OWASP）
- credential は `.wiki/.local/credentials.json`（git 管理外、group/other 権限なし = 0600 以下、sqlite は省略可）から `credential_ref` で解決。パスは全 segment symlink 拒否 + O_NOFOLLOW の同一 fd 検証で読む。秘密値は stdout/stderr/監査/エラーに載せない
