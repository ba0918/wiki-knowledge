...
- **Schema 体制**: v0（`page-template.json`）が schema-of-record。v1（`page-template-v1.json` + `lib/` migrations）は採用トリガー付き standby 資産 — 裁定: `docs/plans/20260707194819_schema-regime-decision.md`
...
- 回答で trust < 0.30 の記事を引用する際は「（信頼度低）」を付す（SKILL.md query 節）
...
- 複数リポジトリは「全 clone → 全 ingest → 一括 compile」の3段で処理（横断 wikilink のため）。手順: SKILL.md「repo ソースの ingest」、compile 規範: `references/compilation-guide.md`「repo ソースの compile」
...
- 読解ガイド: `references/discover-guide.md`、プロンプト: `references/prompts.md` の Discover 節
- パイプライン: `ingest → discover → compile → graph_gen → lint`（discover はオプショナル）
