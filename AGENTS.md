...
- Tool Query の**別系統**。SQL 系（静的検査 + DB role で機械保証）と保証水準が異なる（browser 系 = 封じ込め + 証跡の honest scoping）ため統合せず、catalog スキーマ規約と監査 JSONL 形式のみ共有する。スキル: wiki-browser-extract（SKILL.md 整備は実案件適用サイクルに送る）
