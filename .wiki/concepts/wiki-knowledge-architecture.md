...
Wiki に対して質問する。LLM は関連ページを検索・読み込み、引用付きの合成回答を生成する。重要な洞察：**良い回答は Wiki に新ページとして還元できる** — 探索も知識ベースに複利的に蓄積される。Query 実行時のメタデータは [[querylog]] に構造化ログとして蓄積され、[[gap-detection]] による知識ギャップの検出や [[trust-score]] の基盤となる。
...
graphify 由来の知識グラフ層を中間 derived product として追加する設計案は [[graphify-knowledge-graph-concepts]] を参照。

- [[llm-wiki-tooling]] — Wiki 構築に使うツール群
- [[llm-wiki-use-cases]] — Wiki が活きるユースケース集
