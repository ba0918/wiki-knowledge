...
Wiki に対して質問する。LLM は関連ページを検索・読み込み、引用付きの合成回答を生成する。重要な洞察：**良い回答は Wiki に新ページとして還元できる** — 探索も知識ベースに複利的に蓄積される。Query 実行時のメタデータは [[querylog]] に構造化ログとして蓄積され、知識ギャップの検出やTrust Score の基盤となる。
